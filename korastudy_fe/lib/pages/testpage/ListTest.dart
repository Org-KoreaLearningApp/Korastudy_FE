...
class ListTestWidget extends StatelessWidget {
  ...
  Future<List<Map<String, dynamic>>> _fetchTests() async {
    ...
    for (var doc in snapshot.docs) {
      ...
      var questionsSnapshot = await _firestore.collection('testpage/${doc.id}/questions').get();
      if (questionsSnapshot.docs.isNotEmpty) {
        var questionData = questionsSnapshot.docs.first.data();
        if (questionData.containsKey('createdAt') && questionData['createdAt'] != null) {
  ...
  Widget _buildTestItem(BuildContext context, Map<String, dynamic> test) {
    ...
    String formattedDate = createdAt != null ? DateFormat('dd/MM/yyyy HH:mm').format(createdAt) : 'N/A';

    return Container(
      margin: EdgeInsets.symmetric(horizontal: 16, vertical: 8), // Thêm margin để mục không chiếm toàn bộ chiều rộng
      decoration: BoxDecoration(
  ...
  onPressed: () {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => TestPageListening(testId: test['id']), // Truyền ID của bài kiểm tra
      ),
    );
  },
